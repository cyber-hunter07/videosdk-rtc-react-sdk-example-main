...
const ScreenCaptureToPDF = ({reference}) => {
  const handleCaptureAndSend = async () => {
    try {
      ...
      formData.append('pdf', pdfBlob, 'ScreenCapture.pdf');
      formData.append('recipients', 'vishnuprasadw@gmail.com,vish27W@gmail.com'); // add emails
  ...
  return (
    <div>
      <button style={{color:'whitesmoke'}} onClick={handleCaptureAndSend}>Capture Screen & Save PDF</button>
    </div>
  );
};
